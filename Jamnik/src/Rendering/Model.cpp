...
Model::Model(const char* file)
{
    ...
    bytesText = Jamnik::AssetsRegistry::ReadFile((fileDirectory + uri).c_str());
...
std::vector<unsigned> Model::GetIndices(json accessor)
{
    ...
    if (componentType == 5125)
    {
        for (unsigned int i = beginningOfData; i < byteOffset + accByteOffset + count * 4; i)
        {
            ...
            indices.push_back((unsigned)value);
        }
    }
...
void Model::LoadMesh(unsigned indMesh)
{
    ...
    std::vector<unsigned> indices = GetIndices(JSON["accessors"][indAccInd]);
